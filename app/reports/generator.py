...
def _tender_fact(tender: Tender, score: OpportunityScore | None = None) -> dict:
    return {
        ...
        # Internal support only. AI is explicitly instructed not to treat this as the decision.
...
def generate(db: Session, prospect_id: int, report_type: str = "Two-Page Prospect Preview") -> ReportRun:
    ...
    score_rows = db.scalars(
        select(OpportunityScore).where(OpportunityScore.prospect_id == prospect.id)
    ).all()
    ...
    apply_rows = sorted(
        [r for r in rows if r["recommendation"].recommendation == "APPLY"],
        key=lambda x: x["recommendation"].confidence,
        reverse=True,
    )
    review_rows = sorted(
        [r for r in rows if r["recommendation"].recommendation == "REVIEW"],
        key=lambda x: x["recommendation"].confidence,
        reverse=True,
    )
